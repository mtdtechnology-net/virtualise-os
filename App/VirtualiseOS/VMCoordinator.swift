...
final class Coordinator: NSObject, ObservableObject, URLSessionDownloadDelegate {
    ...
    @Published private(set) var virtualMachineProfiles: [VirtualMachineProfile]
    @Published var selectedProfileID: VirtualMachineProfile.ID? {
        didSet {
    ...
    private var selectedProfile: VirtualMachineProfile? {
    ...
    private var restoreImageDownloadProfileID: VirtualMachineProfile.ID?
    ...
    private static func loadLegacyProfiles() -> [VirtualMachineProfile] {
        guard let data = UserDefaults.standard.data(forKey: profilesUserDefaultsKey) else {
            return []
        }

        do {
            return try JSONDecoder().decode([VirtualMachineProfile].self, from: data)
        } catch {
    ...
    private static func loadSelectedProfileID(from profiles: [VirtualMachineProfile]) -> VirtualMachineProfile.ID? {
    ...
    private func migrateUserDefaultsProfilesIfNeeded() {
        ...
        do {
            let existingCount = try modelContext.fetchCount(FetchDescriptor<VirtualMachineProfileRecord>())
            ...
            legacyProfiles.forEach { modelContext.insert(VirtualMachineProfileRecord(profile: $0)) }
    ...
    private func loadProfilesFromDatabase() {
        ...
        do {
            let descriptor = FetchDescriptor<VirtualMachineProfileRecord>(sortBy: [SortDescriptor(\.createdAt)])
            let records = try modelContext.fetch(descriptor)
            virtualMachineProfiles = records.map(VirtualMachineProfile.init(record:))
    ...
    private func saveProfiles() {
        ...
        do {
            let records = try modelContext.fetch(FetchDescriptor<VirtualMachineProfileRecord>())
            ...
            for profile in virtualMachineProfiles {
                if let record = recordsByID.removeValue(forKey: profile.id) {
                    ...
                    modelContext.insert(VirtualMachineProfileRecord(profile: profile))
                }
            }
    ...
    private func createDefaultProfileIfNeeded() {
        let profile = VirtualMachineProfile(name: "Primary VM".localized,
    ...
    private func updateSelectedProfile(status: VirtualMachineStatus,
                                       detail: String,
                                       progress: Double? = nil,
    ...
    private func refreshProfileStatus(at index: Int) {
        ...
        if profile.isInstalledOnDisk {
            ...
            virtualMachineProfiles[index].statusDetail = "Ready to start.".localized
    ...
    var selectedVirtualMachineProfile: VirtualMachineProfile? {
        selectedProfile
    }
    ...
    func createProfile(name: String,
        ...
        do {
            ...
            let profile = VirtualMachineProfile(name: profileName,
    ...
    func deleteSelectedProfile() {
        ...
        if virtualMachineProfiles[selectedProfileIndex].status == .running {
            showInformationAlert("Stop the virtual machine before removing it from the list.".localized)
            return
        }
    ...
    func selectProfile(_ profile: VirtualMachineProfile) {
    ...
    private func startInAppInstallation() {
        ...
        setupViewModel?.status = "Finding latest supported macOS...".localized
        setupViewModel?.detail = "VirtualiseOS will download the latest macOS restore image supported by this Mac.".localized
        setupViewModel?.progress = 0
    ...
    private func backgroundRestoreImageDownloadSession(for profileID: VirtualMachineProfile.ID) -> URLSession {
    ...
    private func restoreImageDownloadSession(for mode: RestoreImageDownloadMode, profileID: VirtualMachineProfile.ID) -> URLSession {
        switch mode {
        case .foreground:
    ...
    private func startRestoreImageDownload(from url: URL?,
                                           resumeData: Data?,
                                           profileID: VirtualMachineProfile.ID,
